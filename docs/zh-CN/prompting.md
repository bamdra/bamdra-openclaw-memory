...
## USER.md 和 user-bind 的分工

如果你安装的是完整套件，应当把 `bamdra-user-bind` 当成主要的 per-user 画像层。

- 默认称呼、昵称、时区、语气偏好、长期角色设定等稳定用户特征，优先写进绑定画像
- workspace 里的 `USER.md` 应该保持很薄，只保留环境事实、运行上下文或团队共用约束
- 如果当前会话明确要求换一种称呼，以当前会话为准

换句话说，`USER.md` 不应该再承担“这个用户是谁”的长期真相来源，这部分应当交给 `bamdra-user-bind`。
