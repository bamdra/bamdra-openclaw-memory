export type FactCategory = "background" | "environment" | "account" | "security" | "preference" | "project" | "decision" | "constraint";
...
export interface PersistentStore {
    ...
    listFactsByScope(scope: string): Promise<Array<FactRecord & {
        tags: string[];
    }>>;
    listFactsByTags(tags: string[], recallPolicies?: FactRecallPolicy[]): Promise<Array<FactRecord & {
        tags: string[];
    }>>;
    listRecentMessagesForTopic(topicId: string, limit: number): Promise<RecentTopicMessage[]>;
    searchTopics(sessionId: string, query: string, limit: number): Promise<TopicSearchResult[]>;
    searchFacts(args: {
        sessionId: string;
        userId?: string | null;
        query: string;
...
export type TopicRoutingDecision = TopicRoutingDecisionContinue | TopicRoutingDecisionSwitch | TopicRoutingDecisionSpawn;
