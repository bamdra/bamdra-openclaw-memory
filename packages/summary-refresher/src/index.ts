...
export class SummaryRefresher {
  ...
  refresh(input: SummaryRefreshInput): Pick<TopicRecord, "summaryShort" | "summaryLong"> {
    ...
    const openLoops = Array.isArray(input.topic.openLoops) ? input.topic.openLoops : [];
    const latestMessage = recentMessages.at(-1)?.message.text ?? input.topic.summaryShort;
    const factFragments = facts
      .slice(0, 2)
      .map((fact) => `${fact.key}=${fact.value}`)
      .join("; ");
    const loopFragment =
      openLoops.length > 0
        ? ` Open loops: ${openLoops.slice(-2).join(" | ")}.`
        : "";
